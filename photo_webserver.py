...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            #generate_photo.simple(, filename))
            q.enqueue(convert_and_show, os.path.join(app.config['UPLOAD_FOLDER'],filename),result_ttl=0)
            #generate_photo.show_img('./static/'+filename)
            flash("Picture added to display queue.")
